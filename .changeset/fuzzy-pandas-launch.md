...
The nested block accepts a group `name` and an optional `images` array. Each image entry provides an `image` source and Worker `binding`. In that mode, `wrangler deploy` builds and pushes each image, waits while Cloudflare converts it for the Containers runtime, and injects each digest-pinned image reference into its generated Worker binding before upload.
