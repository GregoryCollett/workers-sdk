...
| Change the Vitest integration               | `packages/vitest-pool-workers/`                                       | Vitest integration for tests running in workerd.              |
...
Wrangler, the Vite plugin, and `vitest-pool-workers` are all consumers of the
...
consumer benefits from the same behavior. Keep Wrangler, the Vite plugin, and
`vitest-pool-workers` integration layers thin instead of implementing equivalent
behavior independently in each tool. If Miniflare is not the appropriate layer,
put shared behavior in the lowest suitable shared package.

When adding or changing development functionality:

- assess its impact on Miniflare, Wrangler, the Vite plugin, and
  `vitest-pool-workers`;
...
- Use `vitest-pool-workers` when behavior must be tested in workerd.
...
- New examples under `fixtures/vitest-pool-workers-examples/` need a
  `tsconfig.json`.
