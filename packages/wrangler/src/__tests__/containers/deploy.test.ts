...
describe("wrangler deploy with containers", () => {
	...
	it("should prepare multiple instance images before uploading their generated bindings", async ({
		...
		const namespaceId = "14758f1afd44c09b7992073ccf00b43d";
		const image =
			"registry.cloudflare.com/some-account-id/test-name-sandboxes-sandbox_image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
		const toolsImage =
		...
		try {
			setupDockerMocks(
				expect,
				"test-name-sandboxes-sandbox_image",
				"wrangler-kf12oi"
			);
			fs.writeFileSync("./Dockerfile", "FROM scratch");
			...
			expect(std.out).toContain(
				"Building image test-name-sandboxes-sandbox_image:wrangler-kf12oi"
			);
