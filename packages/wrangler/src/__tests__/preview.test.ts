...
describe("wrangler preview", () => {
	...
	describe("preview command", () => {
		...
		test("should name applications from the resolved worker name, or from the container's own name when set", async ({
			expect,
		}) => {
			writeFileSync(
				"src/index.ts",
		...
		test("should fail before creating the preview deployment when a container omits class_name", async ({
			...
			expect(errorMessage).toContain(
				'The container "my-container" in "previews.containers" is missing "class_name".'
			);
			expect(createDeploymentCalls).toBe(0);
		});
