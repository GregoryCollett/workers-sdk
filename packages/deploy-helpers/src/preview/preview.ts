...
function buildPreviewContainerConfig(
	...
	const linkedContainers = previewContainers.map((container) => {
		...
		if (className === undefined) {
			throw new UserError(
				`The container ${container.name === undefined ? "entry" : `"${container.name}"`} in "previews.containers" is missing "class_name". A preview container must name the Durable Object class it backs, even where a Durable Object declared in "exports" names its container instead.`,
				{
	...
	const filteredContainers = linkedContainers
		...
		.map(({ container, className }) => ({
			...
			name:
				container.name ??
				previewContainerAppName(parentWorkerName, previewSlug, className),
		}));
