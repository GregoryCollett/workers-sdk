...
/**
 * Validate `previews.containers`. Mirrors `validateContainerApp`, but leaves
 * the application name optional. An entry that omits `name` gets one generated
 * at deploy time from the resolved worker name, preview slug, and class name.
 *
...
function validatePreviewsContainers(
	...
	return (diagnostics, field, value, config) => {
		// A Durable Object class is backed by at most one container application,
		// so two entries for the same class are ambiguous. Two entries sharing an
		// application name collide once the containers are applied, so reject
		// both here rather than after the preview deployment exists.
		if (Array.isArray(value)) {
			...
			if (duplicateNames.size > 0) {
				const nameList = [...duplicateNames]
					.map((name) => `"${name}"`)
					.join(", ");
				diagnostics.errors.push(
					`"${field}" declares more than one container with the name ${nameList}; each container application name may appear at most once.`
				);
				return false;
			}
		}
