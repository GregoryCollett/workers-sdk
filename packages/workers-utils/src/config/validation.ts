...
function validateDurableObjectExportContainer(
	...
	if (typeof durableObjectExport.container === "string") {
		...
		if (typeof container.name !== "string" || container.name.length === 0) {
			diagnostics.errors.push(`"${field}.name" must be a non-empty string.`);
			valid = false;
		}
		...
		const allowedProperties = ["name", "images"];
...
function validateContainerExportLinks(
	...
	const classNamesByInstanceGroupName = new Map<string, string[]>();
	...
	for (const [className, entry] of Object.entries(durableObjectExports)) {
		...
		if (isContainerInstanceGroupConfig(entry.container)) {
			...
			classNamesByInstanceGroupName.set(entry.container.name, [
				...(classNamesByInstanceGroupName.get(entry.container.name) ?? []),
				className,
			]);
			continue;
		}
	...
	for (const [groupName, classNames] of classNamesByInstanceGroupName) {
		if (classNames.length > 1) {
			diagnostics.errors.push(
				`The Container Instance Group name "${groupName}" is used by more than one Durable Object export (${classNames.join(", ")}). Instance Group names must be unique.`
			);
		}
	}
